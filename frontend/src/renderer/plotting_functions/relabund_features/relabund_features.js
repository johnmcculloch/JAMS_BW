...
// Listener to open the relabund features pdf when button is clicked
document.getElementById('openRelabundFeaturesLocation').addEventListener('click', () => {
...
window.electron.onParamStr((paramStr) => {
  ...
  // Run plot_relabund_features script
  document.getElementById('relabundFeature-script-form').addEventListener('submit', async (event) => {
  ...
  const featuresImage = document.getElementById('features-image');
  const fileType = result.imagePath.split('.').pop().toLowerCase(); // Get file extension
  if (fileType === 'pdf') {
    await renderPDF(result.imagePath);
  } else {
    featuresImage.src = `file://${result.imagePath}`;
    featuresImage.style.display = 'block';
    featuresImage.onload = () => {
      document.body.scrollTop = document.body.scrollHeight;
    };
  }
  } catch (error) {
  document.getElementById('output').textContent = error;
  document.getElementById('features-image').style.display = 'none';
  }
  });
