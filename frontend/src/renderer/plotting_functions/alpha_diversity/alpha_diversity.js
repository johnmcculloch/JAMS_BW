...
// Listen for submit button to trigger plot_alpha_diversity
document.getElementById('alphaDiversity-script-form').addEventListener('submit', async (event) => {
    ...
    formData.forEach((value, key) => {
      ...
      if (element.type === 'checkbox') {
        ...
        // Split the input string by commas and trim whitespace
        const measuresArray = value.split(',').map(item => item.trim());
        // Convert the array to R vector format
        params[key] = `c(${measuresArray.map(item => `"${item}"`).join(', ')})`;
  ...
  const heatmapImage = document.getElementById('alphaDiversity-image');
  ...
  document.getElementById('alphaDiversity-image').style.display = 'none';
  }
  });
