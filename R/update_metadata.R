...
update_metadata <- function(metadataXL = NULL, phenotable = NULL, phenolabels = NULL, expvec = NULL, maxclass = 10, minGbNAHS = 0, minPctAssembly = 0, readdata = NULL){
    ...
    #Check if samples from old metadata are the same as the new metadata. It is possible to remove samples, but not add, of course.
    oldphenotable <- pData(expvec[[1]])
    Samples_have <- rownames(oldphenotable)

    if (!(setequal(Samples_have, phenotable[, ptsampcol]))){
        flog.warn("Samples in pre-built experiments are different to those in the metadata. Resetting experiments to contain the intersection of samples available and wanted.")
        Samples_intersect <- intersect(Samples_have, phenotable[, ptsampcol])
        phenotable <- phenotable[(phenotable[, ptsampcol] %in% Samples_intersect), ]
    } else {
    ...
    if (!(is.null(readdata))){
        #make sure we are looking at the same samples
        readdata <- readdata[rownames(phenotable), ]
        #Add sequencing info
