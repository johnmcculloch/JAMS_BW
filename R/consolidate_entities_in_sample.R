consolidate_entities_in_sample <- function(opt = opt){
    ...
    if ("MB2bin" %in% names(opt$abundances$taxonomic)){
        flog.info(paste("Finding good quality MetaBAT2 bins at the", curr_quality_level,"level based on modified MIMAG criteria, see https://doi.org/10.1038/s41592-023-01940-w"))

        #Keep bins at HQ and MHQ levels
        if (any(c("HQ", "MHQ") %in% opt$MAGdose$Quality)){
            MetaBAT_bins_to_keep <- opt$MAGdose[which(opt$MAGdose$Quality %in% c("HQ", "MHQ")), "MetaBATbin"]
            surviving_MAGs_df <- opt$MAGdose[MetaBAT_bins_to_keep, ]
            surviving_MAGs_df$BinID <- unname(sapply(surviving_MAGs_df$MetaBATbin, function (x) { tail(unlist(strsplit(x, split = "_")), 1) }))
            surviving_MAGs_df$ConsolidatedGenomeBin <- NA
            #Construct consolidated genome bin name
            for (rn in 1:nrow(surviving_MAGs_df)){
                surviving_MAGs_df[rn, "ConsolidatedGenomeBin"] <- paste("MB2", surviving_MAGs_df[rn, "BinID"], gsub("^LKT__", "", surviving_MAGs_df[rn, "LKT"]), sep = "__")
            }

            #Mark these contigs as consolidated
            opt$contigsdata[which(opt$contigsdata$MetaBATbin %in% MetaBAT_bins_to_keep), "Consolidation_from"] <- "MetaBAT2"

            for (cb in rownames(surviving_MAGs_df)){
                opt$contigsdata[which(opt$contigsdata$MetaBATbin == cb), "ConsolidatedGenomeBin"] <- surviving_MAGs_df[cb, "ConsolidatedGenomeBin"]
            }

            #Consolidate
            surviving_MAGs_df <- surviving_MAGs_df[ , c("ConsolidatedGenomeBin", "Completeness", "Contamination", "Quality", "Completeness_Model_Used", "Taxid", "Domain", "Kingdom", "Phylum", "Class","Order", "Family", "Genus", "Species", "IS1", "LKT", "NCBI_taxonomic_rank", "RefScore", "Gram", "NumBases", "PPM")]
    ...
    for (curr_quality_level in Quality_levels_pecking_order){
        ...
        curr_consolidated_entities <- curr_consolidated_entities[ , c("ConsolidatedGenomeBin", "Completeness", "Contamination", "Quality", "Completeness_Model_Used", "Taxid", "Domain", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species", "IS1", "LKT", "NCBI_taxonomic_rank", "RefScore", "Gram", "NumBases", "PPM")]
