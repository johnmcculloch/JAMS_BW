...
gather_run_info <- function(opt = NULL){
    ...
    Run_info = c("JAMS_version", "JAMS_Kdb_Version", "JAMS_Kdb_Confidence", "Sample_name", "Run_start", "Run_end", "Duration_hours", "CPUs_used")
    Run_value = c(opt$verstr, opt$JAMS_Kdb_Version, as.character(opt$krakenconfidencescore), opt$prefix, as.character(opt$starttime), as.character(opt$endtime), stunde, opt$threads)
