...
get_contig_coverage<-function(opt=NULL, markduplicates=FALSE, classifyunassembled=TRUE, usekraken1=FALSE){
    setwd(opt$workdir)

    #First off, determine if we are going with kraken1 or kraken2
    if(missing(usekraken1) || (usekraken1 != TRUE)){
        ...
        flog.info("Using kraken1. This is deprecated. It is highly suggested that you use a kraken2 database with JAMS.")
        validtaxlvls<-c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "LKT")
    }
    ...
    if("readsdir" %in% names(opt)){
        ...
        if(classifyunassembled==TRUE){
            ...
            if(missing(usekraken1) || (usekraken1 != TRUE)){
                ...
                flog.info("Counting k-mers of unclassified reads with kraken2. Please be patient...")
                krakenargs<-c("--db", opt$workingkrakendb, "--threads", opt$threads, "NAss.fasta")
                ...
                if(hostlength > 0){
                    ...
                    flog.info(paste("Of the non-assebmled reads", hostpct, "% were classified as vertebrate (host) DNA and were disconsidered."))
                }
                ...
                #Get read lengths
                lengthfastacmd<-paste(file.path(opt$bindir, "fasta2length.sh"), "NAss.fasta", ">", "seqlen.map")
                system(lengthfastacmd)
                seqlengths<-read.table(file="seqlen.map", header=FALSE, sep="\t", colClasses = c("character", "numeric"))
                colnames(seqlengths)<-c("Read", "Length")
                file.remove("seqlen.map")
                #Count kmers
                flog.info("Counting k-mers of query sequences with kraken")
                krakenargs<-c("--preload", "--db", opt$workingkrakendb, "--threads", opt$threads, "NAss.fasta", "> tmp.kraken")
                ...
                ##Remove redundancy
                #Get rows which are not duplicated
                krakenbody<-krakendf[,2:ncol(krakendf)]
                uniquerows<-which(!(duplicated(krakenbody)))
                krakenunique<-krakendf[uniquerows,]
                krakenunique<-fix_kraken(taxtable=krakenunique, keepstrain=FALSE)
                colnames(krakenunique)[1]<-"Read"
                krakenunique<-left_join(krakenunique, seqlengths)
 
                #Deal with duplicated rows
                krakendupes<-krakendf[-(uniquerows),]
                krakendupes[] <- lapply(krakendupes, as.character)
                #Sort dulpicated read taxa by kind
                dupkind<-sapply(1:nrow(krakendupes), function (x) { paste0(krakendupes[x,2:9], collapse="_") })

                dupetypes<-data.frame(Read=krakendupes[,1], Type=dupkind)
                dupetypes<-left_join(dupetypes, seqlengths)
                dupetypes<-cbind(dupetypes, krakendupes[, 2:9])
                dupeaggsizes<-aggregate(Length ~ Type, FUN=sum, data=dupetypes)
                dupetypes$Read<-NULL
                dupetypes$Length<-NULL
                dupeaggsizes<-cbind(dupeaggsizes, dupetypes[match(dupeaggsizes$Type, dupetypes$Type) , 2:9])
                dupeaggsizes$Type<-NULL
                dupeaggsizes<-fix_kraken(taxtable=dupeaggsizes, keepstrain=FALSE)
                colnames(dupeaggsizes)[1]<-"Length"
                dupeaggsizes$Read<-paste("NAssRed", 1:nrow(dupeaggsizes), sep="_")
                dupeaggsizes<-dupeaggsizes[, c("Read", "Domain", "Phylum", "Class", "Order", "Family", "Genus", "LKT", "Length")]

                #Join with NAssdata which are not duplicated
                NAssdata<-rbind(krakenunique, dupeaggsizes)
                NAssdata$Length<-as.numeric(NAssdata$Length)
                oriNAsslength<-sum(NAssdata$Length)
                hostlength<-sum(subset(NAssdata, Phylum == "p__Chordata")[]$Length)

                #Reorganize NAss information
                colnames(NAssdata)[which(colnames(NAssdata) == "Read")]<-"Sequence"        
                #Filter out any vertebrate DNA if existant
                NAssdata<-subset(NAssdata, Phylum != "p__Chordata")
                #Report host reads were found, if applicable
                if(hostlength > 0){
                ...
                #Eliminate redundancy and get the dose
                NAssdose<-aggregate(Length ~ LKT, FUN=sum, data=NAssdata)
                colnames(NAssdose)[2]<-"NumBases"
                #Add taxonomic information
                NAsstaxtable<-NAssdata[, validtaxlvls]
                NAsstaxtable<-NAsstaxtable[which(!(duplicated(NAsstaxtable))),]
                NAssdose<-left_join(NAssdose, NAsstaxtable)
                NAssdose$Sequence<-paste("NAssLKT", 1:nrow(NAssdose), sep="_")
                NAssdose<-NAssdose[,c("Sequence", "Domain", "Phylum", "Class", "Order", "Family", "Genus", "LKT", "NumBases")]
                opt$NAssdose<-NAssdose
            } #End conditional using kraken2 or kraken1
        } #End conditional classifying unassembled reads
