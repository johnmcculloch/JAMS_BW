...
get_genomes_NCBI<-function(organisms="bacteria", assembly_summary=NULL, outputdir=NULL, organism_name=NULL, infraspecific_name=NULL, taxid=NULL, species_taxid=NULL, asm_name=NULL, as_general_expression=FALSE, assembly_level=NULL, assembly_upto=NULL, bioproject=NULL, biosample=NULL, refseq_category_upto=NULL, ntop=NULL, nobs=TRUE, fileformat="fasta", simulate=TRUE){
    ...
    if(!is.null(biosample)){
        print(paste("Subsetting to entries only within BioSample(s)", paste0(biosample, collapse=", ")))
    ...
    if((!is.null(species_taxid)) | (!is.null(taxid)) | (!is.null(organism_name))){
        ...
        if(is.null(species_taxid)){
            ...
            print("Subsetting by Organism Name")
            if(as_general_expression != TRUE){
                ...
                print("Considering organism_name supplied to be a general expression. Will select all Organism Names matching the expression(s) supplied.")
            ...
            print("Subsetting by Species Tax ID (species_taxid)")
        ...
        print(paste("There are", nrow(wanted_assembly_summary), "entries matching the Species criteria."))
    }

    #Subset by infraspecies criteria
    if(!is.null(taxid)){
        print("Filtering by Tax ID")
        wantedtaxid<-taxid
        ...
        if(!is.null(infraspecific_name)){
            print("Filtering by strain name")
            strainnames<-infraspecific_name
            wanted_assembly_summary<-subset(wanted_assembly_summary, (infraspecific_name %in% strainnames))
            print(paste("There are", nrow(wanted_assembly_summary), "entries matching the Strain Name criteria."))
        }

        if(!is.null(asm_name)){
            print("Filtering by asm_name")
            asmnames<-asm_name
            if(as_general_expression != TRUE){
                wanted_assembly_summary<-subset(wanted_assembly_summary, (asm_name %in% asmnames))
            } else {
                print("Considering asm_name supplied to be a general expression. Will select all asm names matching the expression(s) supplied.")
            ...
            print(paste("There are", nrow(wanted_assembly_summary), "entries matching the asm name criteria."))
        }
    }
    ...
    print(paste("There are", nrow(wanted_assembly_summary), "entries matching the Assembly Level criteria."))
    ...
    if(!is.null(refseq_category_upto)){
        ...
        print(paste("Subsetting by RefSeq category to include only those within", paste0(wanted_refseq_category, collapse=", "), "categories."))
        wanted_assembly_summary<-subset(wanted_assembly_summary, (refseq_category %in% wanted_refseq_category))
        print(paste("There are", nrow(wanted_assembly_summary), "entries matching the RefSeq criteria."))
    }

    if(nobs == TRUE){
        print("Filtering out entries which have been tagged as being problematic or low quality.")
        noproblem<-which(wanted_assembly_summary$excluded_from_refseq == "")
        wanted_assembly_summary<-wanted_assembly_summary[noproblem, ]
        print(paste("There are", nrow(wanted_assembly_summary), "entries which have NOT been flagged as being problematic or low quality."))
    }
    ...
    if(nrow(wanted_assembly_summary) > 0){
        ...
        print("No genomes fit the criteria imposed.")
