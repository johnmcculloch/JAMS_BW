...
fix_interproscanoutput<-function(opt=NULL){
    ...
    if("iprodir" %in% names(opt)){
        #Get interprojob
        iprojob<-system2('cat', args=file.path(opt$iprodir, "ipro.job"), stdout = TRUE, stderr = FALSE)
        #See if job finished
        iprojobstatus<-system2('sacct', args=c("-j", iprojob), stdout = TRUE)
        #eliminate header
        ...
        while ((ratiojobscomplete < 0.9) && nattempt < 50) {
            ...
            flog.info(paste("The proportion of Interpro jobs complete is currently", ratiojobscomplete))
