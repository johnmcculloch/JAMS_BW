...
define_kinds_of_variables <- function(metadataXL = NULL, phenolabels = NULL, phenotable = NULL, maxclass = 10, maxsubclass = 4){
    ...
    variables_subs<-phenolabels$Var_label[which((phenolabels$Var_type == "subsettable"))]
    ...
    variables_disc<-phenolabels$Var_label[which((phenolabels$Var_type %in% c("discrete", "subsettable")))]
    ...
    variables_bin<-phenolabels$Var_label[which((phenolabels$Var_type != "Sample") & (phenolabels$Var_type != "continuous"))]
    if(length(variables_bin)>0){
        ...
        print("Found no binary variables.")
    }
