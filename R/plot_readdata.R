...
plot_readdata<-function(readdata=NULL){
    ...
    relsize=round(((-1/150)*(nrow(readdata)))+1, 1)
    if(relsize < 0.1){
        relsize<-0.1
    }
    ...
    Bar.reads.QC <- ggplot(readdata, aes(Sample, PctQCpass)) + geom_bar(stat = "identity", fill=(rep(4, length(readdata[,1])))) + labs(title = "Reads passing QC (%)")
    ...
    if(!(is.null(readdata$NonHost_bases))){
        Bar.reads.PctHost <- ggplot(readdata, aes(Sample, PctHost)) + geom_bar(stat = "identity", fill=(rep(4, length(readdata[,1])))) + labs(title = "Bases belonging to Host Species (%)")
        ...
        Bar.reads.NAHS <- ggplot(dfr, aes(Sample, GbNAHS)) + geom_bar(stat = "identity", fill=(rep(4, length(dfr[,1])))) + labs(title = "Gigabytes of usable (non-host) data sequenced")
        ...
        Bar.reads.trim <- ggplot(dfr, aes(Sample, GbTrim)) + geom_bar(stat = "identity", fill=(rep(4, length(dfr[,1])))) + labs(title = "Gigabytes of QC-pass data sequenced")
    ...
    Bar.reads.PctAss <- ggplot(dfr, aes(Sample, PctAss)) + geom_bar(stat = "identity", fill=(rep(4, length(dfr[,1])))) + labs(title = "Bases Assembled into Contigs (%)")
